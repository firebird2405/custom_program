# 장식 에셋 출처·라이선스 (order #1)

포스트잇 보드 꾸미기용 로컬 번들 에셋. 모든 파일은 개인 사용이 허용된 라이선스(CC0 / OFL)로만 수집했으며,
앱 런타임은 이 로컬 파일만 참조한다 (외부 http(s) 요청 0건).
...
- **cork-texture.jpg** — ambientCG "Cork001" 1K PBR 스캔의 Color 맵 (1024×1024 JPEG, 실사 코르크 스캔, seamless/tileable).
  ambientCG의 모든 에셋은 Creative Commons CC0 1.0 Universal (퍼블릭 도메인 헌정)으로 배포된다.
  다운로드 원본: https://ambientcg.com/get?file=Cork001_1K-JPG.zip
- **Gaegu-Regular.ttf** — 개구쟁이(Gaegu), 한글 손글씨 폰트. Google Fonts 공식 저장소(github.com/google/fonts) `ofl/` 디렉터리에서 다운로드.
