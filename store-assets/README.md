...
- **kr/ 캡션 문구 중 '포스트잇' 재검토.** — **해소(2026-08-24, 발주 #33-5).** '포스트잇'은
  3M 등록상표라 설명문·검색어에서는 '스티커 메모' 계열로 먼저 정리했고(발주 #33-4 ·
  `store-description.md` 상단 메모, `protocol/SCORE-AUDIT-2.md` I14 테이크다운 리스크),
  제출 정본인 `kr/` 캡션판도 `tools/captionize-kr.js` CAPTIONS 표를 고쳐 재생성했다:
  09컷 "포스트잇을 5가지 색으로" → "**스티커 메모**를 5가지 색으로", 08컷 "포스트잇에서 📌" →
  "**메모지**에서 📌". `kr/` 6장 전체 일관 재생성(1920×1200·파일명 불변·말줄임 없음 확인).
  스크린샷 속 앱 화면의 "포스트잇 월" 표기는 앱 UI 실물이라 그대로다(촬영 원칙 §4-1).
