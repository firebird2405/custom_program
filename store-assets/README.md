...
- **kr/ 캡션 문구 중 '포스트잇' 재검토.** '포스트잇'은 3M 등록상표라 설명문·검색어에서는
  '스티커 메모' 계열로 정리했는데(발주 #33-4 · `store-description.md` 상단 메모,
  `protocol/SCORE-AUDIT-2.md` I14 테이크다운 리스크), 제출 정본인 `kr/` 캡션판에는
  "포스트잇을 5가지 색으로 어디든 붙여요" 등 상표가 그대로 박혀 있다.
  `tools/captionize-kr.js` 상단 CAPTIONS 표를 '스티커 메모' 계열로 고치고 `kr/` 6장을
  재생성해야 한다 — PNG 재생성은 별도 발주. (미구현)
